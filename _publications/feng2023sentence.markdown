---
layout: publication
title: Sentence Simplification Via Large Language Models
authors: Feng Yutao, Qiang Jipeng, Li Yun, Yuan Yunhao, Zhu Yi
conference: "Arxiv"
year: 2023
bibkey: feng2023sentence
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2302.11957"}
tags: ['Pretraining Methods']
---
Sentence Simplification aims to rephrase complex sentences into simpler sentences while retaining original meaning. Large Language models (LLMs) have demonstrated the ability to perform a variety of natural language processing tasks. However it is not yet known whether LLMs can be served as a high45;quality sentence simplification system. In this work we empirically analyze the zero45;/few45;shot learning ability of LLMs by evaluating them on a number of benchmark test sets. Experimental results show LLMs outperform state45;of45;the45;art sentence simplification methods and are judged to be on a par with human annotators.
