---
layout: publication
title: Promptcblue A Chinese Prompt Tuning Benchmark For The Medical Domain
authors: Zhu Wei, Wang Xiaoling, Zheng Huanran, Chen Mosha, Tang Buzhou
conference: "Arxiv"
year: 2023
bibkey: zhu2023chinese
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2310.14151"}
tags: ['Applications', 'Prompting', 'Tools', 'Training Techniques']
---
Biomedical language understanding benchmarks are the driving forces for artificial intelligence applications with large language model (LLM) back45;ends. However most current benchmarks (a) are limited to English which makes it challenging to replicate many of the successes in English for other languages or (b) focus on knowledge probing of LLMs and neglect to evaluate how LLMs apply these knowledge to perform on a wide range of bio45;medical tasks or (c) have become a publicly available corpus and are leaked to LLMs during pre45;training. To facilitate the research in medical LLMs we re45;build the Chinese Biomedical Language Understanding Evaluation (CBLUE) benchmark into a large scale prompt45;tuning benchmark PromptCBLUE. Our benchmark is a suitable test45;bed and an online platform for evaluating Chinese LLMs multi45;task capabilities on a wide range bio45;medical tasks including medical entity recognition medical text classification medical natural language inference medical dialogue understanding and medical content/dialogue generation. To establish evaluation on these tasks we have experimented and report the results with the current 9 Chinese LLMs fine45;tuned with differtent fine45;tuning techniques.
