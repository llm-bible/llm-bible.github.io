---
layout: publication
title: Integrating A.I. In Higher Education Protocol For A Pilot Study With samcares An Adaptive Learning Hub
authors: Faruqui Syed Hasib Akhter, Tasnim Nazia, Basith Iftekhar Ibne, Obeidat Suleiman, Yildiz Faruk
conference: "Arxiv"
year: 2024
bibkey: faruqui2024integrating
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2405.00330"}
tags: ['Applications', 'Language Modeling', 'RAG', 'Reinforcement Learning', 'Tools']
---
Learning never ends and there is no age limit to grow yourself. However the educational landscape may face challenges in effectively catering to students inclusion and diverse learning needs. These students should have access to state45;of45;the45;art methods for lecture delivery online resources and technology needs. However with all the diverse learning sources it becomes harder for students to comprehend a large amount of knowledge in a short period of time. Traditional assistive technologies and learning aids often lack the dynamic adaptability required for individualized education plans. Large Language Models (LLM) have been used in language translation text summarization and content generation applications. With rapid growth in AI over the past years AI45;powered chatbots and virtual assistants have been developed. This research aims to bridge this gap by introducing an innovative study buddy we will be calling the SAMCares. The system leverages a Large Language Model (LLM) (in our case LLaMa45;2 70B as the base model) and Retriever45;Augmented Generation (RAG) to offer real45;time context45;aware and adaptive educational support. The context of the model will be limited to the knowledge base of Sam Houston State University (SHSU) course notes. The LLM component enables a chat45;like environment to interact with it to meet the unique learning requirements of each student. For this we will build a custom web45;based GUI. At the same time RAG enhances real45;time information retrieval and text generation in turn providing more accurate and context45;specific assistance. An option to upload additional study materials in the web GUI is added in case additional knowledge support is required. The systems efficacy will be evaluated through controlled trials and iterative feedback mechanisms.
