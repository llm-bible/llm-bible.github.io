...
---
Pre45;trained language models have been successful in natural language generation (NLG) tasks. While various decoding methods have been employed they often produce suboptimal results. We first present an empirical analysis of three NLG tasks summarization machine translation and constrained text generation. We found that selecting the best output from the results of multiple decoding methods can significantly improve performance. To further improve reranking for NLG tasks we proposed a novel method textsc123;PairReranker125; which uses a single encoder and a pairwise loss function to jointly encode a source input and a pair of candidates and compare them. Experiments on three NLG tasks demonstrated the effectiveness and flexibility of textsc123;PairReranker125; showing strong results compared with previous baselines. In addition our textsc123;PairReranker125; can generalize to significantly improve GPT45;3 (text45;davinci45;003) results (e.g. 24.5537; on CommonGen and 11.3537; on WMT18 zh45;en) even though our rerankers are not trained with any GPT45;3 candidates.
