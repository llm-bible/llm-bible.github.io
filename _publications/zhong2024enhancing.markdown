---
layout: publication
title: Enhancing Multimodal Large Language Models With Multi45;instance Visual Prompt Generator For Visual Representation Enrichment
authors: Zhong Wenliang, Wu Wenyi, Li Qi, Barton Rob, Du Boxin, Sam Shioulin, Bouyarmane Karim, Tutar Ismail, Huang Junzhou
conference: "Arxiv"
year: 2024
bibkey: zhong2024enhancing
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2406.02987"}
tags: ['Model Architecture', 'Multimodal Models', 'Pretraining Methods', 'Prompting', 'RAG', 'Transformer']
---
Multimodal Large Language Models (MLLMs) have achieved SOTA performance in various visual language tasks by fusing the visual representations with LLMs leveraging some visual adapters. In this paper we first establish that adapters using query45;based Transformers such as Q45;former is a simplified Multi45;instance Learning method without considering instance heterogeneity/correlation. We then propose a general component termed Multi45;instance Visual Prompt Generator (MIVPG) to incorporate enriched visual representations into LLMs by taking advantage of instance correlation between images or patches for the same sample. Quantatitive evaluation on three public vision45;language (VL) datasets from different scenarios shows that the proposed MIVPG improves Q45;former in main VL tasks.
