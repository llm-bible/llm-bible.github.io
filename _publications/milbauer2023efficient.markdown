---
layout: publication
title: LAIT Efficient Multi45;segment Encoding In Transformers With Layer45;adjustable Interaction
...
---
Transformer encoders contextualize token representations by attending to all other tokens at each layer leading to quadratic increase in compute effort with the input length. In practice however the input text of many NLP tasks can be seen as a sequence of related segments (e.g. the sequence of sentences within a passage or the hypothesis and premise in NLI). While attending across these segments is highly beneficial for many tasks we hypothesize that this interaction can be delayed until later encoding stages. To this end we introduce Layer45;Adjustable Interactions in Transformers (LAIT). Within LAIT segmented inputs are first encoded independently and then jointly. This partial two45;tower architecture bridges the gap between a Dual Encoders ability to pre45;compute representations for segments and a fully self45;attentive Transformers capacity to model cross45;segment attention. The LAIT framework effectively leverages existing pretrained Transformers and converts them into the hybrid of the two aforementioned architectures allowing for easy and intuitive control over the performance45;efficiency tradeoff. Experimenting on a wide range of NLP tasks we find LAIT able to reduce 3045;5037; of the attention FLOPs on many tasks while preserving high accuracy; in some practical settings LAIT could reduce actual latency by orders of magnitude.
