---
layout: publication
title: Lightmbert A Simple Yet Effective Method For Multilingual BERT Distillation
authors: Jiao Xiaoqi, Yin Yichun, Shang Lifeng, Jiang Xin, Chen Xiao, Li Linlin, Wang Fang, Liu Qun
conference: "Arxiv"
year: 2021
bibkey: jiao2021simple
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2103.06418"}
tags: ['Applications', 'BERT', 'Distillation', 'Efficiency And Optimization', 'Model Architecture']
---
The multilingual pre45;trained language models (e.g mBERT XLM and XLM45;R) have shown impressive performance on cross45;lingual natural language understanding tasks. However these models are computationally intensive and difficult to be deployed on resource45;restricted devices. In this paper we propose a simple yet effective distillation method (LightMBERT) for transferring the cross45;lingual generalization ability of the multilingual BERT to a small student model. The experiment results empirically demonstrate the efficiency and effectiveness of LightMBERT which is significantly better than the baselines and performs comparable to the teacher mBERT.
