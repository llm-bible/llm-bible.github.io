---
layout: publication
title: Sparsifying Transformer Models With Trainable Representation Pooling
authors: Pietruszka Michał, Borchmann Łukasz, Garncarek Łukasz
conference: "Arxiv"
year: 2020
bibkey: pietruszka2020sparsifying
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2009.05169"}
tags: ['Applications', 'Attention Mechanism', 'Model Architecture', 'Pretraining Methods', 'Training Techniques', 'Transformer']
---
We propose a novel method to sparsify attention in the Transformer model by learning to select the most45;informative token representations during the training process thus focusing on the task45;specific parts of an input. A reduction of quadratic time and memory complexity to sublinear was achieved due to a robust trainable top45;k operator. Our experiments on a challenging long document summarization task show that even our simple baseline performs comparably to the current SOTA and with trainable pooling we can retain its top quality while being 1.8× faster during training 4.5× faster during inference and up to 13× more computationally efficient in the decoder.
