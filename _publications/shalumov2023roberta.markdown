---
layout: publication
title: Hero Roberta And Longformer Hebrew Language Models
authors: Shalumov Vitaly, Haskey Harel
conference: "Arxiv"
year: 2023
bibkey: shalumov2023roberta
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2304.11077"}
tags: ['Applications', 'BERT', 'Model Architecture', 'Pretraining Methods', 'Transformer']
---
In this paper we fill in an existing gap in resources available to the Hebrew NLP community by providing it with the largest so far pre45;train dataset HeDC4 a state45;of45;the45;art pre45;trained language model HeRo for standard length inputs and an efficient transformer LongHeRo for long input sequences. The HeRo model was evaluated on the sentiment analysis the named entity recognition and the question answering tasks while the LongHeRo model was evaluated on the document classification task with a dataset composed of long documents. Both HeRo and LongHeRo presented state45;of45;the45;art performance. The dataset and model checkpoints used in this work are publicly available.
