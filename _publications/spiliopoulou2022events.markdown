---
layout: publication
title: Events Realm Event Reasoning Of Entity States Via Language Models
authors: Spiliopoulou Evangelia, Pagnoni Artidoro, Bisk Yonatan, Hovy Eduard
conference: "Arxiv"
year: 2022
bibkey: spiliopoulou2022events
additional_links:
  - {name: "Paper", url: "https://arxiv.org/abs/2211.05392"}
tags: ['Pretraining Methods', 'Prompting', 'Reinforcement Learning']
---
This paper investigates models of event implications. Specifically how well models predict entity state45;changes by targeting their understanding of physical attributes. Nominally Large Language models (LLM) have been exposed to procedural knowledge about how objects interact yet our benchmarking shows they fail to reason about the world. Conversely we also demonstrate that existing approaches often misrepresent the surprising abilities of LLMs via improper task encodings and that proper model prompting can dramatically improve performance of reported baseline results across multiple tasks. In particular our results indicate that our prompting technique is especially useful for unseen attributes (out45;of45;domain) or when only limited data is available.
